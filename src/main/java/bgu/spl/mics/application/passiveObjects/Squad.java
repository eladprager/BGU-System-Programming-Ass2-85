package bgu.spl.mics.application.passiveObjects;
import java.sql.Time;
import java.util.HashMap;
import java.util.LinkedList;
import java.util.List;
import java.util.Map;

/**
 * Passive data-object representing a information about an agent in MI6.
 * You must not alter any of the given public methods of this class. 
 * <p>
 * You may add ONLY private fields and methods to this class.
 */
public class Squad {

	private Map<String, Agent> agents;

	private static class SingeltonSquad{
		private static Squad sq = new Squad();
	}

	private Squad(){
		agents = new HashMap<>();
	}

	/**
	 * Retrieves the single instance of this class.
	 */
	public static Squad getInstance() {
		return SingeltonSquad.sq;
	}

	public Map<String, Agent> getAgentsMap() {
		return agents;
	}

	/**
	 * Initializes the squad. This method adds all the agents to the squad.
	 * <p>
	 * @param agents 	Data structure containing all data necessary for initialization
	 * 						of the squad.
	 */
	public void load (Agent[] agents) {
			for (int i = 0; i < agents.length; i++) {
				this.agents.put(agents[i].getSerialNumber(), agents[i]);
			}
	}

	/**
	 * Releases agents.
	 */
	public void releaseAgents(List<String> serials){
		synchronized (this) {
			for (int i = 0; i < serials.size(); i++) {
				if (this.agents.containsKey(serials.get(i))) {
						this.agents.get(serials.get(i)).release();
					this.notify();
					}
				}
			}
		}


	/**
	 * simulates executing a mission by calling sleep.
	 * @param time   milliseconds to sleep
	 */
	public void sendAgents(List<String> serials, int time){
	//	synchronized (this) {
			try {
				for (int i=0;i<serials.size();i++){
					agents.get(serials.get(i)).acquire();
				}
				Thread.sleep((100 * time));
				releaseAgents(serials);

			} catch (InterruptedException e) {
				e.printStackTrace();
			}
		//}
	}

	/**
	 * acquires an agent, i.e. holds the agent until the caller is done with it
	 * @param serials   the serial numbers of the agents
	 * @return ‘false’ if an agent of serialNumber ‘serial’ is missing, and ‘true’ otherwise
	 */
	public boolean getAgents(List<String> serials){
		synchronized (this) {
			for (int i = 0; i < serials.size(); i++) {
					if (!this.agents.containsKey(serials.get(i))) {
						return false;
					}
					try {
						while (!this.agents.get(serials.get(i)).isAvailable()) {
							wait();
						}
					} catch (InterruptedException ignored) {
					}
				}
			}
			for (int i = 0; i < serials.size(); i++) {
				this.agents.get(serials.get(i)).acquire();
			}
			return true;
	}



    /**
     * gets the agents names
     * @param serials the serial numbers of the agents
     * @return a list of the names of the agents with the specified serials.
     */
    public List<String> getAgentsNames(List<String> serials) {
			List<String> names = new LinkedList<>();
			for (int i = 0; i < serials.size(); i++) {
				if (agents.containsKey(serials.get(i))) {
					names.add(this.agents.get(serials.get(i)).getName());
				}
			}
			return names;
	}

}
